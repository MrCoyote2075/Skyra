const { app, BrowserWindow, ipcMain, BrowserView, screen, clipboard } = require('electron');
...
const ALLOWED_DOMAINS = ["hackerearth.com", "is.gd", "accounts.google.com", "oauth"];
...
// Focus tracking (Triggers the 5-sec timer)
app.on('browser-window-blur', () => {
  if (!examStarted || isExiting) return;

  const activeWindow = BrowserWindow.getFocusedWindow();
  if (activeWindow) return; // OK if viewing an internal popup
  ...
  blurTimer = setTimeout(() => {
    console.log("❌ 6 Seconds up. Terminating.");
    isExiting = true;
    app.quit();
  }, 6000); // Updated to 5000ms
});

// Focus regained (Stops timer, shows Post-Warning)
app.on('browser-window-focus', () => {
  ...
  // We do NOT restore the view yet. We show the Post-Warning first.
...
ipcMain.on('start-exam', (event, code) => {
  if (screen.getAllDisplays().length > 1) {
    mainWindow.webContents.send('show-error', "Multiple monitors detected! Disconnect external displays.");
    return;
  }
  ...
  view.webContents.on('devtools-opened', () => view.webContents.closeDevTools());
  ...
  view.webContents.on('will-navigate', (e, navUrl) => {
    const isAllowed = ALLOWED_DOMAINS.some(domain => navUrl.includes(domain));
...
// View Visibility toggles (Used for Exit Modal & Post-Warning Resume)
...
// Called when they click "I understand" on the post-warning
...
ipcMain.on('refresh-exam', () => { if (view) view.webContents.reload(); });
ipcMain.on('exit-exam', () => { isExiting = true; app.quit(); });